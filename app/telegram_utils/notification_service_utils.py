...
def add_new_user(telegram_api: TelegramAPI, telegramID: str, rule: str, minimum_value: str) -> str:
    """
    ...
    Args:
        telegram_api (TelegramAPI): Instance of the TelegramAPI used to send user configuration.
    ...
    uuid = ''
    if rule == 'rarity':
        uuid = telegram_api.add_to_queue(f'|ADD|{telegramID}|NEXT|M{minimum_value}')
    elif rule == 'ranking':
        uuid = telegram_api.add_to_queue(f'|ADD|{telegramID}|NEXT|T{minimum_value}')
    
    return uuid


def add_new_hash(telegram_api: TelegramAPI, hash: Hash, top: int) -> str:
    """
    ...
    Args:
        telegram_api (TelegramAPI): Instance of the TelegramAPI used to send hash notifications.
    ...
    uuid = telegram_api.add_to_queue(f'|NEW|{hash.word}|NEXT|{hash.isFromBeginning}|NEXT|{hash.hashType}|NEXT|{hash.counts}|NEXT|{hash.user}|NEXT|{hash.createdAt}|NEXT|{top}')
    return uuid


def remove_user(telegram_api: TelegramAPI, telegramID: str) -> str:
    """
    ...
    Args:
        telegram_api (TelegramAPI): Instance of the TelegramAPI used to send user removal requests.
    ...
    uuid = telegram_api.add_to_queue(f'|REM|{telegramID}')
    return uuid


def change_rule(telegram_api: TelegramAPI, telegramID: str, rule: str, minimum_value: str) -> str:
    """
    ...
    Args:
        telegram_api (TelegramAPI): Instance of the TelegramAPI used to send user rule changes.
    ...
    uuid = ''
    if rule == 'rarity':
        uuid = telegram_api.add_to_queue(f'|CHN|{telegramID}|NEXT|M{minimum_value}')
    elif rule == 'ranking':
        uuid = telegram_api.add_to_queue(f'|CHN|{telegramID}|NEXT|T{minimum_value}')
    
    return uuid


async def wait_until_response(telegram_api: TelegramAPI, message_uuid: str, max_time_in_seconds: int = 5) -> int:
    """
    ...
    Args:
        telegram_api (TelegramAPI): An instance of the TelegramAPI class used to query the message queue.
    ...
    if not telegram_api.is_open():
        return -1
    
    messages = await telegram_api.wait_for_matching_message(message_uuid, timeout=max_time_in_seconds)
    if messages is None:
        return -1
